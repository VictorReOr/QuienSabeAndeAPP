    ...
    return (
        <div className={index % 2 ? "grid grid-flow-row auto-rows-auto bg-gray-50" : "grid grid-flow-row auto-rows-auto bg-gray-100"}>
            <div className="flex cursor-pointer">
                <div className="w-full p-4" key={bill.name} onClick={() => methods.selectBill(bill)}>
                    <div className="flex items-center">
                        ...
                        <div className="flex-1 min-w-0 ms-4">
                            <p className={"text-sm font-medium truncate " + (selectedBill === bill.name ? "text-blue-500 dark:text-blue-500 font-bold" : "text-gray-900 dark:text-white")}>
                                {!isEditing ? billName.current : <input name='Nombre cuenta' ref={inputName} defaultValue={billName.current} type='text' onBlur={blurHandler} />}
                ...
                <button className="w-fit p-4" onClick={() => setIsEditing(!isEditing)}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                        <path strokeLinecap="round" strokeLinejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
                    </svg>
                </button>
                <button className="w-fit p-4" onClick={handleDelete}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
            ...
            <div className={isExpanded ? 'flex justify-center' : 'hidden '}>
                <ul className="w-fit divide-y divide-gray-200 dark:divide-gray-700">
                    {groupedProducts && Object.keys(groupedProducts).map((name: string) => (
                        <li className='p-2 text-sm font-medium truncate text-gray-900 dark:text-white' key={name}>
                            <div className="grid grid-cols-3 gap-4 text-center">
                                ...
                                <p className='w-fit	'>x{groupedProducts[name].length}</p>
                                <p className='w-fit	font-semibold'>{groupedProducts[name].length * groupedProducts[name][0].price} €</p>
                            </div>
                        </li>
                    ))}
                </ul>
